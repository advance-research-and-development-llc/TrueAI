...
| Coverage — statements | 63.64% | **81%** |
| Coverage — branches | 54.56% | **73%** |
| Coverage — functions | 54.05% | **73%** |
| Coverage — lines | 65.77% | **83%** |
| `npm run build:dev` | ✅ | ✅ |
...
- [x] 2.8 App shell (`App.tsx`, routing, lazy-load fallbacks) — `src/App.routing.test.tsx` adds 12 tests covering `isTabName` guard (3 branches: valid, invalid string, wrong type), top-level `handleTabChange` routing for all 6 tabs, tab-trigger hover handlers, header icon-button dialog open paths (Settings, Customize), `createConversation` create + cancel flows, mobile shell (`useIsMobile=true`) MobileBottomNav + chat-tab FloatingActionButton conditional, and the agents-tab sub-tab walk + `createAgent` dialog flow including `toggleAgentTool`. Took `src/App.tsx` from 21.5/14.8/5.3/26.6 (stmt/br/fn/lines) to 29.9/22.9/18.0/34.5. Heavy event handlers (`runAgent`, `executeWorkflow`, `sendMessage`, `triggerLearning`) deferred to post-decomposition slices per Phase 4.
- [x] 2.9 Leaf-hook branch density — **`src/hooks/use-streaming-chat.ts`** L 94.83→**96.55%**, B **68.18→86.36%** (+18 pp), F→**100%** (+5 tests: system-prompt prepend, mid-stream `error` part via `streamText({ onError })` callback, non-Error onError payload wrapped, non-Error throw from `model.doStream` wrapped in catch, and abort-during-send via never-ending `ReadableStream` that errors on `AbortSignal`). **`src/hooks/use-contextual-ui.ts`** L 94.39→**100%**, B 67.74→**100%** (+12 tests: parametrized time-of-day buckets for morning/afternoon/evening/night, plus a null-`behavior` describe block that overrides the `useKV` mock to return `null` to exercise every `prev ?? initialBehavior` and `prev || []` fallback in `trackFeatureUsage`/`trackTimeOfDay`/`trackError`/`trackSessionDuration`/`dismissSuggestion` and the `if (!behavior)` early returns in `generateSuggestions`/`getPredictedNextAction`/`getRecommendedFeatures`). Global lines 83.37→**83.44%**, branches 73.27→**73.52%**, statements 81.00→**81.12%**. Ratcheted statements floor 80→81 (lines/functions/branches floors unchanged at 83/73/73).
