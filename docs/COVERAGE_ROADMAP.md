...
- [ ] 2.6 Interactive components (chat export, settings, workflow, install
      prompt, error boundaries) — **HarnessUploadUI**: 18.8→**93.8%** stmt / 27.6→**96.6%** br / 6→21/21 fn (dialog open/close, mode switch, URL+manifest submits, toggle/remove, tools split). **CollaborativeAgentManager**: 29.5→**90.9%** stmt / 21.8→**90.0%** br / 5→20/20 fn (toggleAgent, preview banner gating, success+error onRunCollaboration via deferred Promise, getAgentName fallback).
